...
class ServiceTwoModule(Module):
    ...
    def configure(self, binder: Binder) -> None:
        service_one_client = ServiceOneClient(self.service_two)
        binder.bind(ServiceOneClient, to=service_one_client, scope=singleton)


def configure_views(app):
    @app.route('/get_hello/<name>')
    def get_hello(name: str, service_one_client: ServiceOneClient):
